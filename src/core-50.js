...
function oatrRefreshCore50Ui() {
  ...
  if (folio) {
    ...
    if (number && q) number.textContent = String(q.coreNumber || ((state?.index || 0) + 1)).padStart(2, '0');
    if (total) total.textContent = `OF ${OATR_CORE_TOTAL}`;
  }
  ...
  if (bar && typeof state !== 'undefined') {
    ...
    bar.style.width = `${Math.max(0, Math.min(100, Math.round(state.index / denom * 100)))}%`;
  }
}
